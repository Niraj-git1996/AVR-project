/* total time of wave form is 100ms of which 25ms is ON and 75ms is OFF the timer1 is 16bit counter which can count 0000-FFFF
...
int main(void)
{
   DDRB|=(1<<DDB2);   /// led port b as output
    ...
    {
        PORTB|=led;
        Timer1(1);    ///25ms  ON time
        PORTB&=(~led);   /// toggle led
        Timer1(3);     ///75ms OFF time
    }
