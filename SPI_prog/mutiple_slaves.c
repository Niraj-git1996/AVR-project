/*  The Serial Programming Interface (SPI) is support full duplex communication these used $ pin MISO,MOSI,SS,SCLK.In atmega
     ...
     data to slave.
     */
/// The MAX7219 are connected multiple with AVR controllers as a slave the master low particular cc pin os slave
/// and write data int it.
#include <avr/io.h>
#include <util/delay.h>   /// delay to swuch between character
#define F_CPU  16000000UL  /// f_cpu

#define sck (1<<PINB5)
#define mosi (1<<PINB3)
#define ss1 (1<<PINB2)   ///slaves 1
#define ss2 (1<<PINB1)   /// slaves 2
#define  ss3 (1<<PINB6)   /// slaves 3



void CE1(unsigned char cmd,unsigned char data)  /// IC 1 used the command and data to write led matrix
{
    PORTB&=(~ss1);
    SPDR=cmd;
...
void CE2(unsigned char cmd,unsigned char data)  /// IC 2 used the command and data to write led matrix
{
    PORTB&=(~ss2);
    SPDR=cmd;
    while(!(SPSR & (1<<SPIF)));  // wait until transmission is complete
...
void CE3(unsigned char cmd,unsigned char data)  /// IC 3 used the command and data to write led matrix
{
    PORTB&=(~ss3);
    SPDR=cmd;
    while(!(SPSR & (1<<SPIF)));  // wait until transmission is complete
...
int main(void)
{
  ...
  // uint8_t C[]={0X38,0X44,0X04,0X04,0X04,0X04,0X44,0X38};
   ...
   uint8_t  alpha[10][9]={{0x3C,0X42,0X42,0X42,0X7E,0X42,0X42,0X42},{0X3C,0X44,0X44,0X44,0X3C,0X44,0X44,0X3C},
                          ...
                          ,{0X3C,0X02,0X02,0X02,0X3E,0X02,0X02,0X3C}};///char array with A,B,C,D,E,F  command word
    // Insert code
   init_spi();
   //execute(0x01,0x0F);
   //execute(0x02,0x05);
